          ...
          {[0, 1].map((i) => (
            <span
              ...
              style={{
                ...
                transition: "top 240ms cubic-bezier(0.22,1,0.36,1), transform 240ms cubic-bezier(0.22,1,0.36,1), background 200ms ease",
              }}
            />
          ))}
        </span>
      ...
      >
        <div style={{ padding: "8px 24px 40px" }}>
          ...
          <div className="flex flex-col" style={{ gap: 10, marginTop: 28 }}>
            <button
              type="button"
              style={{ padding: "13px 0", fontSize: 13, lineHeight: "20px", color: INK, border: `1px solid ${RULE}` }}
            >
              Log In
            </button>
            <button
              type="button"
              style={{ padding: "13px 0", fontSize: 13, lineHeight: "20px", color: "#FFFFFF", background: INK }}
            >
              Book a Demo
            </button>
          </div>
...
function Row({ item, onNavigate }: { item: MenuItem; onNavigate: () => void }) {
  ...
  const body = (
    <>
      <span
        aria-hidden
        style={{ width: 6, height: 6, marginTop: 7, flexShrink: 0, background: dot ?? RULE, display: "block" }}
      />
      <span>
        <span style={{ display: "block", fontSize: 14, lineHeight: "20px", color: item.to ? INK : MUTED }}>
          {item.short ?? item.title}
        </span>
        <span style={{ display: "block", marginTop: 2, fontSize: 12, lineHeight: "18px", color: FAINT }}>
          {item.desc}
        </span>
      </span>
    </>
...
function Plus({ open }: { open: boolean }) {
  return (
    <span aria-hidden style={{ position: "relative", display: "block", width: 10, height: 10 }}>
      <span style={{ position: "absolute", top: 4.5, left: 0, width: 10, height: 1, background: MUTED }} />
      <span
        style={{
          position: "absolute",
          top: 4.5,
