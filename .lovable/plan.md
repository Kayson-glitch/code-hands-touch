...
## 改动内容
1. **增大噪声幅度**
   - `GOOEY_NOISE` 从 `0.011` 提升到 `0.016` 左右。
   - 低频次低频偏移（`seed` 驱动的 blob）幅度从 `2×` 提高到 `3×` 左右。

2. **叠加高频空间噪声**
   - 在 `distorted` 计算中加入一层高频 hash 噪声：使用 `fract(sin(seed * 45.7) * 123.45)` 等 hash 函数，幅值约 `GOOEY_NOISE * 0.5`。
   - 再加入一层更高频的微小抖动（`GOOEY_NOISE * 0.25`），让边缘在细胞级别出现细碎断裂。
